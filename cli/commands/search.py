    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for modality in ["text", "image", "video", "audio"]:
                ...
                print(
                    f"[_search_api] {modality.capitalize()} results count: {len(results)}"
                )
...
"""
    ...
    def on_mount(self) -> None:
        ...
        results_list.add_option(
            Option(prompt="[dim]Enter at least 3 characters to search[/dim]", id="")
        )
    ...
    def _perform_search(self, query: str) -> None:
        ...
        self.log(
            f">>> Got results for modalities: {list(self.results_by_modality.keys())}"
        )
        ...
        if results:
            ...
            for result_index, result in enumerate(results):
                ...
                filename = result.get("filename") or (
                    Path(path).name if path else "Unknown"
                )
                ...
                if self.current_modality == "audio" and max_score > 0:
                    ...
                    score_display = (
                        f"{raw_score:.2f}" if raw_score >= 0.01 else f"{raw_score:.2e}"
                    )
                display_text = f"[bold]{filename}[/bold]{cloud_tag}{relevance_tag} [dim]{score_display}[/dim]"
                results_list.add_option(
                    Option(prompt=display_text, id=str(result_index))
                )
            results_list.highlighted = 0  # Highlight first
        else:
            results_list.add_option(
                Option(prompt=f"[dim]No {self.current_modality} results[/dim]", id="")
            )
    ...
    def handle_selection(self, event: OptionList.OptionSelected) -> None:
        ...
        try:
            ...
            if 0 <= index < len(self.current_results):
                ...
                self.log(
                    f">>> Opening item: {selected_item.get('filename', 'unknown')}"
                )
                self._open_fn(selected_item)
            else:
                self.log(
                    f">>> Invalid index: {index}, current_results length: {len(self.current_results)}"
                )
